...
export function general_shape(globals, scale, inner_point_list, surface_point_list, spring_data_list, frame_index_lists_collection, is_frame_fixed_collection, damping) {
    ...
    shape.all_vertices = surface_vertex_list.concat(inner_vertice_list);
    ...
    shape.particle_container = new ParticleContainer({
        dynamicProperties: {
            position: true,
            scale: false,
            rotation: false,
    ...
    for (let vertex of shape.all_vertices) {
        let particle = make_particle(globals, vertex.x, vertex.y);
        shape.particle_container.addParticle(particle);
